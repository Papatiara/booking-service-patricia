...
class Booking extends Component {
  ...
  componentDidMount() {
    this.getRoomListing(Math.floor(Math.random() * (99)) + 1000000);
  }
  ...
  getRoomListing(id) {
    axios.get(`/booking/${id}`)
      .then(({ data }) => {
        const bookings = data.results.reservations.rows;
        const listing = data.results.accomodations.rows[0];
        const owner = data.results.owner.rows[0];
        const reviews = data.results.reviews.rows[0];
        console.log(data.results.reviews.rows[0])
  ...
  onSubmitTripDetailsWithDates() {
    const { price, cleaningfee, areaTax, accomodations_id } = this.state.listing;
              ...
              <div id="booking-form">
                <div className="booking-form-fields">
                  <div className="form-fields-spacing-dates">
                    <div>
                      <div id={`dates_${this.state.listing}`} className="form-fields-label">
                      ...
                      <BookingCalendar
                        ...
                        bookedDates={this.state.bookings.map(b => moment.range(b.from_date, b.end_date))}
                ...
                <PricingQuote
                  ...
                  cleaningFee={this.state.listing.cleaingfee}
